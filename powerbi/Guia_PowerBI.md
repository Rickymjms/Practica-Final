...
*   **Periodo_Anio:** Número entero (No resumir).
*   **Presupuesto_Inicial, Presupuesto_Vigente, Devengado_Aprobado:** Moneda (RD$).
...
```DAX
-- 1. Totales Base
Total Inicial = SUM('PresupuestoNacionalRD'[Presupuesto_Inicial])
Total Vigente = SUM('PresupuestoNacionalRD'[Presupuesto_Vigente])
Total Devengado = SUM('PresupuestoNacionalRD'[Devengado_Aprobado])
...
*   **Año Fiscal:** `Periodo_Anio` (Lista o Menú desplegable).
*   **Sección:** `Seccion_Institucional`.
*   **Jerarquía Funcional:** `Finalidad` > `Funcion` > `Sub_Funcion`.
*   **Jerarquía Organizacional:** `Capitulo` > `Sub_Capitulo`.
...
*   **Gráficos de Variación:** Dos gráficos de barras horizontales mostrando el **Top 5 Aumentos** y **Top 5 Disminuciones** por `Capitulo`.
